...
// include: F:\VSCode\data\tmp\tmpi6bawiby.js
...
// end include: F:\VSCode\data\tmp\tmpi6bawiby.js
...
var ASM_CONSTS = {
  623416: () => { if (document.fullscreenElement) return 1; },  
 623462: () => { return document.getElementById('canvas').width; },  
 623514: () => { return parseInt(document.getElementById('canvas').style.width); },  
 623582: () => { document.exitFullscreen(); },  
 623609: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 623682: () => { if (document.fullscreenElement) return 1; },  
 623728: () => { return document.getElementById('canvas').width; },  
 623780: () => { return screen.width; },  
 623805: () => { document.exitFullscreen(); },  
 623832: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 623965: () => { return window.innerWidth; },  
 623991: () => { return window.innerHeight; },  
 624018: () => { if (document.fullscreenElement) return 1; },  
 624064: () => { return document.getElementById('canvas').width; },  
 624116: () => { return parseInt(document.getElementById('canvas').style.width); },  
 624184: () => { if (document.fullscreenElement) return 1; },  
 624230: () => { return document.getElementById('canvas').width; },  
 624282: () => { return screen.width; },  
 624307: () => { return window.innerWidth; },  
 624333: () => { return window.innerHeight; },  
 624360: () => { if (document.fullscreenElement) return 1; },  
 624406: () => { return document.getElementById('canvas').width; },  
 624458: () => { return screen.width; },  
 624483: () => { document.exitFullscreen(); },  
 624510: () => { if (document.fullscreenElement) return 1; },  
 624556: () => { return document.getElementById('canvas').width; },  
 624608: () => { return parseInt(document.getElementById('canvas').style.width); },  
 624676: () => { document.exitFullscreen(); },  
 624703: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 624761: () => { return screen.width; },  
 624786: () => { return screen.height; },  
 624812: () => { return window.screenX; },  
 624839: () => { return window.screenY; },  
 624866: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 624919: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 624990: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 625047: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 625303: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 625374: () => { if (document.fullscreenElement) return 1; },  
 625420: () => { return window.innerWidth; },  
 625446: () => { return window.innerHeight; },  
 625473: () => { if (document.pointerLockElement) return 1; },  
 625520: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 627678: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 627842: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 627946: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 630945: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 631011: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 631404: ($0) => { miniaudio.untrack_device_by_index($0); },  
 631447: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 631572: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
