...
// include: F:\VSCode\data\tmp\tmpam88d9jk.js
...
// end include: F:\VSCode\data\tmp\tmpam88d9jk.js
...
var ASM_CONSTS = {
  420248: () => { if (document.fullscreenElement) return 1; },  
 420294: () => { return document.getElementById('canvas').width; },  
 420346: () => { return parseInt(document.getElementById('canvas').style.width); },  
 420414: () => { document.exitFullscreen(); },  
 420441: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 420514: () => { if (document.fullscreenElement) return 1; },  
 420560: () => { return document.getElementById('canvas').width; },  
 420612: () => { return screen.width; },  
 420637: () => { document.exitFullscreen(); },  
 420664: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 420797: () => { return window.innerWidth; },  
 420823: () => { return window.innerHeight; },  
 420850: () => { if (document.fullscreenElement) return 1; },  
 420896: () => { return document.getElementById('canvas').width; },  
 420948: () => { return parseInt(document.getElementById('canvas').style.width); },  
 421016: () => { if (document.fullscreenElement) return 1; },  
 421062: () => { return document.getElementById('canvas').width; },  
 421114: () => { return screen.width; },  
 421139: () => { return window.innerWidth; },  
 421165: () => { return window.innerHeight; },  
 421192: () => { if (document.fullscreenElement) return 1; },  
 421238: () => { return document.getElementById('canvas').width; },  
 421290: () => { return screen.width; },  
 421315: () => { document.exitFullscreen(); },  
 421342: () => { if (document.fullscreenElement) return 1; },  
 421388: () => { return document.getElementById('canvas').width; },  
 421440: () => { return parseInt(document.getElementById('canvas').style.width); },  
 421508: () => { document.exitFullscreen(); },  
 421535: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 421593: () => { return screen.width; },  
 421618: () => { return screen.height; },  
 421644: () => { return window.screenX; },  
 421671: () => { return window.screenY; },  
 421698: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 421751: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 421822: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 421879: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 422135: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 422206: () => { if (document.fullscreenElement) return 1; },  
 422252: () => { return window.innerWidth; },  
 422278: () => { return window.innerHeight; },  
 422305: () => { if (document.pointerLockElement) return 1; },  
 422352: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 424510: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 424674: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 424778: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 427777: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 427843: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 428236: ($0) => { miniaudio.untrack_device_by_index($0); },  
 428279: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 428404: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
...
var _Keyboard_PressBreak = Module['_Keyboard_PressBreak'] = () => (_Keyboard_PressBreak = Module['_Keyboard_PressBreak'] = wasmExports['Keyboard_PressBreak'])();
